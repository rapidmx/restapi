...
export class MailboxMongo extends BaseMongoEntity implements Mailbox {
    ...
    @Description(
        "This mailbox's Blocked Senders list: lowercase addresses (`user@example.com`) and domains (`@example.com`). Mail from one " +
            ...
            "and only by the mailbox's owner or a full-access delegate. A row without the field reads as empty.",
    )
    @Nullable
    ...
    @Description(
        "This mailbox's Safe Senders list, shaped like `blockedSenders`. Authenticated mail from one is never junked for a spam " +
            "verdict (it never releases mail an antivirus or policy verdict quarantined). A row without the field reads as empty.",
    )
    @Nullable

...
- **`POST /messages/:id/report` reports a message as junk, phishing or not junk, and teaches rspamd from it.** The body is `{ kind: "junk" | "phishing" | "not_junk", alwaysTrustSender?: boolean }` and the answer `{ uid, kind, moved, folderUid, learned, learnSkipped?, safeSender? }`. It moves the message on the server (`junk` and `phishing` to the mailbox's Junk Email folder, `not_junk` to its Inbox - either is created when missing; a message already there is not moved, `moved: false`; Drafts and Outbox are a `400`), records what was reported on the new server-managed `Message.reportedAs` (`"junk"` | `"phishing"` | `"not_junk"`) and `Message.dateReported` (nullable columns on SQL, so `synchronize` adds them with no migration; no create or update body can set them), and audits it as `message.reported` (`details`: `kind`, `from`, `moved`, `folderUid`, `learned`, `learnSkipped`, `alwaysTrustSender`) - a phishing report is the same entry with `kind: "phishing"`; this library has no administrator alert channel, but the entry is also emitted as a telemetry event of that type. It needs READ and UPDATE on the message's folder (`403`), the message must exist (`404`) and `kind` must be one of the three (`400`); it is limited to 600 requests a minute per user and is idempotent (reporting again moves nothing and audits again). The spam filter is taught the raw message (`junk` and `phishing` as spam, `not_junk` as ham) through the new optional `SpamScanProvider.learn(raw, "spam" | "ham", { recipient? })`, best-effort: the move stands whatever it does, and `learned: false` says why in `learnSkipped` - `encrypted` (the server cannot read the message: nothing is read or sent), `disabled` (`mail:scan:spam:learn:enabled` is `false`; default `true`), `unsupported` (no engine registered, or one without `learn()`), `too_large` (the source is over 5 MiB, `MAX_REPORT_LEARN_BYTES`: nothing is sent) or `failed` (the engine was unreachable, refused, timed out, or the source could not be read; logged). `alwaysTrustSender: true` (`not_junk` only, else `400`) also adds the `From` header's address - read from the message's source, else `Message.from.address` - to the mailbox's safe senders, taking it off the blocked list, and answers it as `safeSender`; it needs full access to the mailbox (`403` before anything is done), since the safe list is an owner-level setting.
- **`RspamdSpamScanProvider.learn()` posts to rspamd's controller.** `POST <controller>/learnspam` or `/learnham` with the raw message as an `application/octet-stream` body, the controller password as the `Password` header and the reporting mailbox's address as `Deliver-To` (what a per-user classifier files it under); any 2xx counts (rspamd answers a message it already learned with 208) and anything else, a network failure or the timeout rejects. Three new settings: `mail:scan:spam:rspamd:controller_url` (default empty = the scan worker's `mail:scan:spam:rspamd:url` with its port replaced by 11334, the controller's port beside the scan worker's 11333), `mail:scan:spam:rspamd:controller_password` (default empty: no header, which rspamd accepts from its `secure_ip`) and `mail:scan:spam:rspamd:controller_timeout_ms` (default 10000). rspamd itself needs a statistics backend (Redis) configured before it can learn anything.
- **`Mailbox.blockedSenders` and `Mailbox.safeSenders` - Outlook's Blocked Senders and Safe Senders lists,** `simple-json` nullable columns on SQL and plain fields on Mongo (no migration; a row from before them reads as `[]` everywhere, including in `GET /mailboxes` and `GET /mailboxes/:id`). An entry is a plain lowercase address (`user@example.com`) or a domain (`@example.com`, the exact domain and not its subdomains; `example.com` is stored as `@example.com`); at most 1,000 entries of at most 254 characters, repeats removed, and an entry is never on both lists. They are owner-level settings, guarded like `freeBusyVisibility`: changing either by `PUT`, property `PUT`, bulk `PUT` or create needs full access to the mailbox (the owner, or a "manager" delegate - a delegate with plain update access gets `403`, and an administrator with no grant cannot touch them), an unchanged list or `null` passes so a full-object round trip works, and an invalid or oversized list, or an entry named on both lists, is a `400`. `POST /mailboxes/:id/blocked-senders` and `POST /mailboxes/:id/safe-senders` (body `{ entry }`) and `DELETE /mailboxes/:id/blocked-senders/:entry` and `/safe-senders/:entry` (the entry URL-encoded, e.g. `%40example.com`) add or remove ONE entry atomically - a version-checked write that re-reads and re-applies the change when another write won the lock (up to 5 times, then `409`), so two browser tabs never overwrite each other - and answer `{ entry, changed, blockedSenders, safeSenders }`; adding an entry to one list removes it from the other, adding a present entry or removing an absent one changes nothing, and a caller who cannot read the mailbox gets `404` and one without full access `403`.
- **`ScanQueueJob` enforces the lists.** Mail whose `From` address OR envelope sender is on the blocked list (the address, or its exact domain, case-insensitively, no authentication needed) is filed in Junk Email like any junk-routed mail - the mailbox's rules do not run - and its scan is marked with the spam symbol `BLOCKED_SENDER`. Mail whose `From` address is on the safe list and authenticated (`verifiedFromAddress()`: a passing DKIM signature aligned with the `From` domain, stamped by the trusted MTA hop) is never junked for a spam verdict - including the spam engine's fail-closed outage verdict - but delivered to the Inbox, where the rules and Focused Inbox run as usual, marked `SAFE_SENDER`; an unauthenticated message from a safe address is not rescued, the envelope sender is not consulted for the safe list, and an address on both lists (only possible by editing rows directly) is safe when authenticated and blocked when not. Neither list ever changes an antivirus or policy quarantine.
- **Mail filter rules can match a sender exactly: `MailFilterConditions.fromEquals` and `fromDomainEquals`.** `fromEquals` is a list of plain addresses, matched case-insensitively and exactly (`ann@x.com` does not match `joann@x.com`, which `fromContains` still does) against the `From` header's address OR the SMTP envelope sender; `fromDomainEquals` is a list of domains matched exactly against the domain of either (`x.com` is not `mail.x.com`). They are validated and normalized on create and update (each entry a plain address or a domain of at most 254 characters, at most 100 entries, lowercased, de-duplicated; a domain's leading `@` removed; `null` dropped; anything else a `400`). `MailFilterMatchContext` gains optional `fromAddress` and `envelopeFrom`; without either, the address inside `from` is used.

### Fixed

- **Permanently deleting messages left their attachments and blobs behind.** `DELETE /messages/:id?purge=true` and `DELETE /messages?folderUid=` removed only the message rows and their search documents, orphaning every `Attachment` row and every blob (the raw message, the sanitized HTML, superseded draft bodies, attachment content and extracted text) - unlike `RetentionEnforcementJob`. They now delete the attachments and each blob no other row references (inbound mail is stored once for every recipient and a filter rule's copies share theirs, so a blob another message, attachment, quarantine entry or undelivered ingest entry - soft-deleted rows included - still names is kept), after the messages are gone, in bounded batches, best-effort: a row or blob that cannot be deleted is logged and never fails the delete. A subclass that does not set both `quarantineEntryClass` and `ingestQueueEntryClass` (the Mongo and SQL routes do) keeps the raw message and HTML, since it cannot tell whether another recipient still needs them. `MessagePurgeUtils` (`collectMessagePurge()`, `finishMessagePurge()`) holds the logic; `BaseScopedChildRoute` gains the `beforePurge()`/`afterPurge()`/`afterTruncate()`/`onTruncateRefused()` hooks and its `checkLegalHold()` receives the caller.

### Changed

- **Emptying a folder is audited.** `DELETE /messages?folderUid=` writes one `message.truncate` entry per call (`targetType` `Folder`, `targetUid` the folder, `details: { folderUid, count }`, the actor), none for an empty folder, and a truncate refused by a legal hold writes `legal_hold.blocked_delete` with `details: { folderUid, truncate: true, count }` like a refused purge. `AuditAction` gains `MESSAGE_REPORTED` and `MESSAGE_TRUNCATE`.
- **The legal hold's `409` no longer lists the blocking matters' uids for an ordinary user.** The status and `api-011` code are unchanged; the text is `This action is blocked by an active legal hold.` for a caller without a trusted role, who has no business learning that an investigation exists, and still names the matters for one with a trusted role. (`assertNotOnLegalHold()`, used by the mailbox delete and the erasure requests, keeps its text.)
...
- **An administrator can erase the data a deleted mailbox left behind, which frees its address for reuse.** Deleting a mailbox removes only its row and its own access list; its folders (with their access lists) and everything carrying its `mailboxUid` stayed, and `POST /mailboxes` at the same address answered `409 This address still has data from a deleted mailbox. Erase that data before reusing the address.` with no way to do what it said - an ordinary erasure request is self-service and needs the mailbox row. Three additions, all for a trusted AND elevated caller (else `403` `api-103`/`api-104`, as `?scope=admin`):
    - `GET /mailboxes/leftover[?limit=&after=]` lists the deleted mailboxes that still have data as `{ items: [{ mailboxUid, folderCount, messageCount, erasure? }], next? }`, sorted by uid (`limit` defaults to 50, at most 100; `next` is the `after` for the following page and is absent at the end; `erasure` is the newest erasure request filed for the address, `{ uid, status, dateCreated }`). It is the distinct `mailboxUid`s of the folders (soft-deleted ones included) that no mailbox has as its uid; reads are bounded (100 uids at a time, at most 5,000 read per call, then a cursor) and carry metadata only. An address whose only remains are an access list is not listed, but is still refused on create and can be erased by naming it. Audited as `mailbox.admin-list` with `details.leftover`.
    - `POST /erasure-requests/leftover` with `{ "mailboxUid": "<address>" }` files an erasure of that address's data, already approved (a `DataSubjectErasureRequest` with the new `leftoverOnly: true`, `requestedByUserUid` and `reviewedByUserUid` the administrator), so `ErasureExecutionJob` runs it like any other: the same purge of every `mailboxUid`-scoped row and blob, legal-hold checks and audit trail (`erasure_request.created`, `.approved` and `.completed`, the first two with `details.leftover`). The answer is the request; poll `GET /erasure-requests/:id` until its `status` is `completed` (`approved` and `in_progress` are waiting and running). Refused: `400` for a missing or implausible `mailboxUid`; `409` with `reason: "mailbox-exists"` when the mailbox row exists (a live mailbox is erased only through an ordinary erasure request); the legal-hold `409` while an open matter names the address; `404` when there is nothing left (no folder and no access list). Repeating it returns the request already approved or running rather than a second one, and a still-`pending` request for the address (a former owner's own) is approved instead of duplicated. An address is trimmed and lowercased first, as when a mailbox is created.
    - `DELETE /mailboxes/:id?erase=true` deletes the mailbox and files that same erasure in one step (administrator only - anybody else is refused with `403` before anything is deleted). Any delete of a mailbox that leaves data now says so in `X-Mailbox-Data: kept`, or `X-Mailbox-Data: erasing` with `X-Erasure-Request: <request uid>` when `?erase=true` filed the erasure; if filing it fails the mailbox stays deleted and the answer is `kept`.
- **The `409` on creating a mailbox at an address with leftover data carries a machine-readable `reason`**: `mailbox-data-remaining` (with `mailboxUid`: offer the erasure) or `mailbox-data-erasing` (with `erasure: { uid, status }`: an approved or running erasure has not finished, which also holds while its folders are already gone so a mailbox created meanwhile cannot meet the job's last steps). The message and `api-011` code are unchanged.

### Changed

- **`ErasureExecutionJob` erases a deleted mailbox's data completely.** With no mailbox row it now also removes the mailbox's own `AccessControlList` document (it is what keeps the address taken) after purging everything else, and never when a mailbox row is back at the address; a failure to remove it hands the request back for a retry instead of completing. A request filed with `leftoverOnly` never removes a mailbox row: if one exists when it runs, the request is denied (`erasure_request.denied`, `details.automatic`) and nothing is purged. The final legal-hold re-check now applies to a mailbox that is already gone too. Every purge matches the mailbox uid (and a calendar share link's folder uid) as an exact query literal, never a parsed operator or pattern.
...
- **"Find a time" can see other people's busy times: `POST /calendar-events/free-busy`.** The body is `{ addresses, start, end }` - one to 50 plain addresses (trimmed, lowercased and de-duplicated; a name with the address, a list of lists or anything else is a 400) and an ISO 8601 window of at most 31 days with `end` after `start` - and the answer is `{ start, end, results: [{ address, status, busy: [{ start, end, tentative }] }] }`, one result per address in the order asked. `status` is `available` (with the busy windows over the window: sorted, merged where they overlap or touch, clipped to it, never a title, location, attendee, organizer or id, so a private or confidential event counts as busy like any other), `restricted` (a mailbox that exists but whose owner does not share it with the caller) or `unknown` (not a local mailbox or alias - external free/busy is not looked up - or a calendar too large to read within the bounds: three queries of at most 2,000 rows read in 500-row pages, as `ScanQueueJob` bounds a resource booking, or more than 500 windows, is reported `unknown` rather than as free time nobody checked). The windows come from every calendar folder of the mailbox through `computeBusyWindows()`'s expansion (recurring series, exceptions and moved occurrences, in the event's own time zone), skipping cancelled, free and deleted events and events in other folders, and skipping an event the mailbox owner declined; a window is `tentative` when the event's `busyStatus` is tentative or the owner has not answered the invitation or only answered tentatively (their answer on the attendee entry, ignored for an event they organize), and a firm event wins over a tentative one where they overlap. A caller who owns no mailbox on this server is told `unknown` for a mailbox they may not see, exactly as for one that does not exist, because the address directory (`GET /mail/directory`) would not have shown it to them either (403): `restricted` is only ever said to somebody who could already list the mailbox (a trusted role counts, as in the directory, and never sees more). The route is on `BaseCalendarEventRoute`, so a server picks it up with its existing calendar-events route (`POST /api/mail/calendar-events/free-busy`); a caller who is not signed in gets 401, a signed-in caller is limited to 60 requests a minute, and the SQL route overrides the new `aliasQueryValue()` as the other routes do.
- **`Mailbox.freeBusyVisibility` (`"domain"` | `"shared"` | `"nobody"` | `"everyone"`, default `"domain"`): each user chooses who may see their free/busy.** `domain` is any signed-in user who owns a mailbox in the same domain as the mailbox's primary address; `shared` is only callers who already hold access (read, list or `freebusy`) on the mailbox or one of its calendar folders; `nobody` is only the owner and delegates with full access; `everyone` is any signed-in user on the server. The owner and full-access delegates always see their own mailbox whatever it says, and a caller who holds read or list access sees it under `domain` too, since they can read the events themselves. It is a `varchar` column on SQL and a plain field on Mongo, so `synchronize` adds it and a row written before it existed (absent on Mongo, `null` on SQL) reads as `domain` everywhere, including in what `GET /mailboxes` and `GET /mailboxes/:id` return. Changing it takes full access to the mailbox - the owner's own record or a "manager" delegate's - so a delegate with plain update access cannot widen it (403), an administrator with no grant of their own cannot change it (it is not one of the administrative fields, so a `PUT` drops it and `PUT /:id/freeBusyVisibility` is 403), an unchanged value or `null` passes so a full-object round trip keeps working, and a value that is not one of the four is a 400, on create and update alike. `FreeBusyLookupUtils` holds the lookup, the request validation and the limits, and `FreeBusyUtils` gains `computeBusyIntervals()` (`computeBusyWindows()` with `tentative` and the owner's answer, which it now calls) and `mergeBusyIntervals()`.

- **A message's meeting invitation can be answered, added or removed from the message itself, as in Outlook.** `GET /calendar-events/invite/:messageUid` reads the calendar file in a message (a `text/calendar` part or an `.ics` attachment, read from the stored raw message, never from anything a client says) and reports what it is (`REQUEST`, `REPLY`, `CANCEL`, `PUBLISH`, `COUNTER`, or a file with no `METHOD`, read as a `PUBLISH`), who organized it, whether it is already on the reader's calendar, what they answered, and which of `canRespond`, `canPropose`, `canAdd`, `canRemove` and `canAcceptProposal` apply; it also returns the reader's own `schedule` from 12 hours before to 12 hours after the meeting and the busy `conflicts` inside it. `POST .../respond` (`accepted`, `tentative`, `declined`) puts the meeting on the calendar at the same deterministic uid inbound processing uses (so the two never make two copies) or updates the answer on the copy that is there, removes it on a decline, and mails the organizer an iTIP `REPLY` either way; `PUBLISH` files are added on Accept and mail no one. `POST .../remove` takes a cancelled meeting off the calendar. `POST .../propose` mails the organizer a `COUNTER` with the proposed start, end and an optional comment, and `POST .../accept-proposal` moves a meeting the reader organizes to the time a listed attendee proposed, bumps its `SEQUENCE` so the scheduling job re-invites everyone, and resets the others' answers. Answering your own invitation, a cancellation or a reply is a `400`. Before this an invitation from a sender that isn't DKIM-verified, or a plain `.ics`, was just a file to download.
- **`Message.meetingMethod` and `Message.meetingResponse`** (server-managed): the iTIP `METHOD` of the calendar file a message carries, set when it is delivered, so a message list can show an RSVP button without opening every message (a conversation row carries the most recent message's as `latestMeetingMethod` and `latestMeetingResponse`), and what the mailbox owner answered, kept on the message because a decline leaves nothing on the calendar to read it from.
- **`MeetingInviteUtils`** (`describeInvite()`, `parseInviteIcs()`, `extractIcsFromRaw()`, `conflictsOf()` and friends) and `buildEventIcs()` accepts `COUNTER`.

- **A calendar event has a rich-text description: `CalendarEvent.descriptionHtml` (sanitized HTML) and `CalendarEvent.description` (its plain text).** The server sanitizes the HTML on every create and update, whatever the client sent (`sanitizeEventDescriptionHtml()`, new in `util/EventDescriptionUtils.ts`, far stricter than the mail sanitizer): only `b`/`strong`, `i`/`em`, `u`, `br`, `p`, `ul`/`ol`/`li` and `a` survive, the only attribute is a link's `href` - `http`, `https` or `mailto` only - written with `rel="noopener noreferrer"`, and every other attribute, `style`, image, script and `javascript:`/`data:`/`tel:` link is removed (a link that goes nowhere safe becomes its text; `script`/`style`/`iframe`/`svg`/`object`/`textarea` go with their content; the output is escaped, balanced and unchanged by sanitizing it again). A write giving only `descriptionHtml` gets `description` derived from it (line breaks for paragraphs, breaks and list items, `- `/`1. ` before a list item, a link's URL after its text); one giving only plain `description` keeps it and clears the HTML; both given keeps both; `null` or an empty string clears. `description` over 32,000 characters, `descriptionHtml` over 64,000 (as sent and as sanitized), or a value that isn't a string is a `400`. The description travels in an invitation as `DESCRIPTION` (escaped) and `X-ALT-DESC;FMTTYPE=text/html` - the HTML sanitized again when one is received - is filed on the guest's copy, is mailed in the invitation's body and read from a message by `GET /calendar-events/invite/:messageUid` (`description`, `descriptionHtml`). `buildEventIcs()` now folds every content line longer than 75 octets (RFC 5545 §3.1), never inside a multi-byte character, so an invitation whose lines are all short is 7bit rather than quoted-printable.
- **`CalendarEvent.visibility` (`"default"` | `"public"` | `"private"` | `"confidential"`, default `"default"`; an existing row reads as `"default"`): a private or confidential event is shown to a reader who is not the owner only as a busy block.** A shared-calendar reader without `UPDATE` on the calendar, or the holder of a calendar share link, gets such an event from `GET /calendar-events` and `GET /calendar-events/:id` with the title `"Busy"`, `attendees: []`, an empty `organizer.address`, `redacted: true` (a response-only marker) and no location, description, video meeting, reminder, automatic reply, guest permissions or scheduling bookkeeping - the time, recurrence, status, busy status and `visibility` stay, so the block lands where it belongs and a series still expands. The owner and delegates with `UPDATE` see everything, and `default` and `public` events are shown in full to every reader. Such a reader can't filter or sort by `title`, `location`, `description`, `attendees` and the other hidden fields (a `400`, on `HEAD` too), since a filter would give a private title away. A create or update of such an event is published on the calendar's live-update channel as the same block (`redacted: true`, since every subscriber, a read-only grantee included, gets one payload), so the owner's client refetches an event whose notification says `redacted`. It travels as `CLASS` (`PUBLIC`/`PRIVATE`/`CONFIDENTIAL`; none for `default`) and a guest's copy keeps what the organizer sent. `redactEventForReader()` and the other helpers are in `util/CalendarEventUtils.ts`.
- **Guest permissions on a calendar event: `guestsCanModify` (default `false`), `guestsCanInviteOthers` (default `true`), `guestsCanSeeGuestList` (default `true`).** They travel in the invitation as `X-RAPIDMX-GUESTS-CAN-MODIFY`, `-CAN-INVITE` and `-CAN-SEE-GUEST-LIST` (`TRUE`/`FALSE`; a value at its default is left out) and are stored on each guest's copy. With `guestsCanSeeGuestList: false`, `MeetingSchedulingJob` mails each guest an invitation - and a cancellation - naming only that guest and the organizer, one message per guest as it already does for a linked video meeting (an event with the default takes the shared-message path exactly as before), the guest's copy lists only themselves, `GET /calendar-events/invite/:messageUid` lists only the reader for a guest, and the organizer's own row still lists everyone. Editing the three fields on a copy the mailbox does not organize is ignored (not refused, so a full object still round-trips); a change to any of them, to the description or to the visibility raises `sequence`, so guests are re-invited.
- **`POST /calendar-events/:id/request-change`: a guest asks the organizer to change an event, or to add guests.** The body is `{ title?, location?, description?, descriptionHtml?, startDate?, endDate?, addAttendees?: [{ address, displayName? }] }`. The caller must be able to update their own copy, be a listed guest and not the organizer (`400`); their copy's flags decide (`403` with the reason): a change of the title, location, description or time needs `guestsCanModify`, guests to add need `guestsCanInviteOthers`. A value equal to what the event has is not a change and a request that changes nothing is a `400`; `title` and `location` are non-empty and at most 1,000 characters (a request can set them, not clear them), `addAttendees` has at most 50 plain addresses and the event at most 500 guests. It mails the organizer an iTIP `COUNTER` (`buildEventIcs(..., "COUNTER", { changeRequest: true, extraAttendees })`) carrying the proposed values, `X-RAPIDMX-CHANGE-REQUEST:TRUE` and the requested guests after the requester's own `ATTENDEE`, and changes nothing locally; the id names the caller's own row, so a series or a single occurrence's override row can be changed and a bare occurrence with no row of its own can't. `502` when it can't be mailed. When the organizer's mailbox is a RapidMX one, `ScanQueueJob` applies the request itself - from a DKIM-verified sender who is a listed guest, for an event that mailbox organizes and is not cancelled, made of a revision no older than the row's - if the organizer's **own row's** flags allow every change in it (what the message says its flags are is ignored, and a request with one change not allowed applies none): the changed fields are written, new guests are added as required needs-action guests (deduplicated, plain addresses, at most 50 a request and 500 in all), `sequence` is raised so `MeetingSchedulingJob` re-invites everyone, and - only when the time changed - the other guests go back to needs-action and the requester is accepted, as accepting a proposal does. The delivered message is marked with `Message.meetingResponse: "accepted"` (as accepting a proposal marks it). A `COUNTER` without the marker, or one the flags don't allow, stays the proposal it was, which `POST .../accept-proposal` still moves the time for; `Propose new time` is open to every guest whatever the flags.
- **`GET /calendar-events/invite/:messageUid` reports the event dialog's fields.** `MessageInvite` gains `description`, `descriptionHtml` (sanitized), `visibility`, `guestPermissions` (`{ guestsCanModify, guestsCanInviteOthers, guestsCanSeeGuestList }`), `canRequestChange` and `canRequestInvite` (a `REQUEST` already on the reader's calendar, the reader not the organizer, whose organizer allows it - the buttons to offer, posting to `/calendar-events/<calendarEventUid>/request-change`) and, on a guest's change-request `COUNTER`, `changeRequest: { applied }`; `canAcceptProposal` is `false` once such a request was applied.
...
- Added tests for previously uncovered paths: the mid-stream quota-limit 413 in `create()`, and `MailboxImportJob`'s best-effort source-blob delete failing after a request is attempted or abandoned.

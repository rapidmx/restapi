...
### 2026-09-24 - Free/busy for "Find a time": `POST /calendar-events/free-busy` and `Mailbox.freeBusyVisibility`

JP's decision: each user picks, in their settings, who may see their free/busy (default: same domain). The new-event dialog's "Find a time" needs other people's busy windows, and nothing exposed them (the share links' `freebusy` action is anonymous and per folder; reading events needs a real ACL grant).

- **Setting.** `Mailbox.freeBusyVisibility`: `domain` (default) | `shared` | `nobody` | `everyone`. Mongo plain field defaulting to `"domain"`; SQL `varchar`, `nullable` so `synchronize` can add it to existing rows (no migration), which then read `null`. Every reader goes through `effectiveFreeBusyVisibility()` (`?? "domain"`), and `withAccessRole()` normalizes it on `GET /mailboxes[/:id]` so a client never sees `null`. Validated with `assertFreeBusyVisibility()` on create and in `validateUpdate()` (400). A real change needs `FULL` on the mailbox (`hasMailAccess`, so never a trusted role): the owner's record or a "manager" delegate's. Delegates with plain update access (custom role) cannot widen it; an admin with no grant never reaches it (`restrictToAdminFields()` drops it; the property PUT is refused by the existing `ADMIN_MANAGED_FIELDS` check). An unchanged value or `null` passes, so a full-object PUT round trip works.
- **Meanings, precisely** (`mayView()` in `util/FreeBusyLookupUtils.ts`): the caller's own mailbox (`ownerUserUid`) is always visible; `everyone` admits anyone signed in; `nobody` needs `FULL` (a manager delegate); `domain` admits a caller who OWNS a mailbox whose primary address has the same domain (the domain of an alias address does not count, nor an alias domain - `Domain.aliasOf` is not resolved here) OR holds `read`/`list`/`freebusy` on the mailbox or one of its calendar folders - a deliberate superset of JP's wording, because such a caller can read the events anyway and refusing them would only be confusing; `shared` needs that access. "Holds a mailbox" means owns one: a caller whose only access is as a delegate of a shared mailbox is in no domain.
- **Privacy decision: `unknown` versus `restricted`.** The directory (`BaseDirectoryRoute`) lists every mailbox to any user who owns one here (or holds a trusted role) and answers everybody else 403, so `restricted` reveals nothing new to the first group and would be an existence oracle for the second (an identity from the shared auth service with no mailbox here). For a caller who is not in the first group, a mailbox that exists but does not admit them is therefore reported `unknown`, identical to a missing one; `restricted` is only said to those who could list it. Trusted roles count as in the directory but widen nothing (`hasMailAccess()` strips them). `everyone` mailboxes are reported `available` to any signed-in caller, by their owner's own choice. Also bounded: 50 addresses, 31 days, 60 requests a minute per user (`@RateLimit`, like the directory's).
- **What it computes.** `computeBusyIntervals()` (new in `FreeBusyUtils`; `computeBusyWindows()` now calls it and is unchanged for its callers): the same expansion (recurrence, exceptions, moved occurrences excluded from their master, per-event time zone), skips cancelled and free events, skips an event whose attendee entry for one of the mailbox's addresses is `declined`, marks tentative for `busyStatus: tentative` and for `needsAction`/`tentative` answers, ignoring the owner's entry for an event they organize (the organizer address matches or `isOrganizer`). `mergeBusyIntervals()` sweeps the edges: overlapping and touching windows of the same kind join, firm wins over tentative where they overlap, everything is clipped to the window. Private/confidential events are treated like any other; the response has no titles, locations, attendees or ids (a test asserts the response keys and that none of the event's text appears).
- **Reads are bounded** the way `ScanQueueJob.readBookingPages()` bounds them (`FREE_BUSY_LIMITS`: 500-row pages, 4 pages per query, three queries: overlapping rows, recurring masters, moved occurrences from a day before the window; 500 merged windows). Overflowing any bound reports `unknown` for that mailbox - never a partial answer that would read as free. `FREE_BUSY_LIMITS` is a mutable object only so the tests can lower it. Calendar folders: `CALENDAR` type, not soft-deleted, at most 50; a row counts only if its `folderUid` is one of them.
- **Known limits.** External (other-server) addresses are `unknown` - no federation of free/busy, and no `Domain.aliasOf` resolution of a typed alias-domain address. Working hours are not modelled (a mailbox reads as free outside its events). `RepoUtils.find()` leaves soft-deleted rows out, so a soft-deleted moved occurrence no longer stands for its master's original occurrence (the same behavior as `loadSchedule()` in the invite endpoints; a cancelled one is a row and does). Resource mailboxes are ordinary mailboxes here (visible per their own setting, `domain` by default). Event `visibility` (another change) is deliberately not read: private events still count as busy.
- **Placement.** A method on `BaseCalendarEventRoute` (declared before the `/:id` routes; `POST /free-busy` collides with none) so the server inherits it with its `CalendarEventRoute` and no route class or `mailAccessRouteTable` row is added; only `TRUSTED_ROLE_USES` gained `BaseCalendarEventRoute.ts` (the `isTrusted()` that decides `restricted` versus `unknown`). The SQL subclass adds `aliasQueryValue()` (LIKE with escaped `%`/`_`, as `MailboxAccessRouteSQL`).
- **Tests.** `test/routes/freeBusySuite.ts` runs on both backends from `{mongo,sql}/FreeBusyRoute.test.ts`: all four modes against the owner, a colleague, another domain, read access on the mailbox, `freebusy` access on the folder only, update-only, read+update, a full-access delegate, a caller with no mailbox and a trusted role with none; 401; unknown/alias/dedupe; recurrence/exception/moved/cancelled/free/deleted/other folders; declined/tentative; clipping and merging; every 400; the bounds; and the who-may-change rules through PUT, property PUT, bulk PUT and create. `test/util/FreeBusyIntervals.test.ts` and `test/util/FreeBusyLookupUtils.test.ts` cover the pure parts.

### 2026-09-24 - The event dialog's data: rich description, visibility, guest permissions, and a guest's request to change an event

JP: the Google-Calendar-style event dialog (web-client) needs description (rich text), visibility and guest permissions. Decisions were made up front (sanitized HTML, `CLASS`, Google-style flags); this records how they landed and what is NOT enforced.

- **Fields** (all on `CalendarEvent`, Mongo + SQL pair, not server-managed): `description?`, `descriptionHtml?` (SQL `text`, nullable), `visibility` (`"default"|"public"|"private"|"confidential"`, SQL `varchar` nullable), `guestsCanModify` (false), `guestsCanInviteOthers` (true), `guestsCanSeeGuestList` (true) (SQL booleans nullable). `nullable: true` and `@Nullable` because the columns were added to populated tables (no `DEFAULT` in `@Column`) and a client that reads a legacy SQL row and writes it back sends `null` - `validateEventPolicyFields()` drops a `null`, and every reader goes through `effectiveVisibility()`/`guestPermissionsOf()` (`util/CalendarEventUtils.ts`), never the raw column. `redacted?` is a response-only marker on the interface, not a column.
- **Sanitizer** (`util/EventDescriptionUtils.ts`): `htmlparser2` tokenizer + our own writer, like `scan/HtmlSanitizer` but with a far smaller allow-list (`b strong i em u br p ul ol li a`, only `a[href]`, `http`/`https`/`mailto` - `sanitizeLinkUrl()` minus its `tel:`). It re-runs until the output stops changing (max 4 passes) because unwrapping an element can change how the rest nests (`<p><span><p>` -> nested `p`s the parser then splits); nesting depth 32, 2,000 elements, input cut at 4x the bound. Reuses `MailUrlRules` (the URL rules) rather than adding a dependency; `sanitize-html` is in `package.json` but the repo's own mail sanitizer is what everything else uses. `normalizeEventDescription()` is the single decision for a write (see the release note for the rules); `sanitizeInboundDescription()` is the lenient twin for an inbound `.ics` (cuts/drops instead of a 400). **Gotcha:** a client-sent `description` is kept when both fields come, so the plain text can disagree with the HTML - deliberate (Outlook sends both and its plain text is its own rendering).
- **iCalendar.** `DESCRIPTION` (escaped TEXT), `X-ALT-DESC;FMTTYPE=text/html` (escaped as TEXT too; both sanitized again on build and on parse), `CLASS` (omitted for `default`), `X-RAPIDMX-GUESTS-CAN-MODIFY:TRUE`, `-CAN-INVITE:FALSE`, `-CAN-SEE-GUEST-LIST:FALSE` (each only when it differs from its default, so an update back to the defaults says so by omission), `X-RAPIDMX-CHANGE-REQUEST:TRUE`. Only `REQUEST`/`CANCEL`/change-request `COUNTER` carry them. **`buildEventIcs()` now folds every line over 75 octets** (`foldLine()`, UTF-8 safe): a long description needs it, and as a side effect an invitation whose lines are all short is 7bit instead of quoted-printable (`MeetingSchedulingJob{Mongo,SQL}` tests that looked for `=3D` were loosened).
- **Visibility enforcement** is a read-side redaction, not a query filter: `BaseScopedChildRoute` got three hooks, `redactReadRecords()` (after `find()`/`findById()`), `assertQueryAllowed()` (before `find()`/`count()`) and `pushPayload()` (what `notify()` publishes), no-ops by default; `BaseCalendarEventRoute` overrides them. **Live updates:** every subscriber of a folder's push channel gets the same payload, and a read-only grantee may subscribe, so a create/update notification of a private/confidential event carries the busy block (`redacted: true`) - the owner's client must refetch an event whose notification says `redacted` instead of storing the payload. "Sees everything" = `hasMailAccess(effectiveUser, folderUid, UPDATE)` (owner has `FULL`; a delegate with `UPDATE` too; a shared-calendar grantee with `read`/`list` and a share-link identity do not). The block keeps time, recurrence, status, busy status, `icalUid` and `visibility`, and drops everything else (see the release note). `queryNamesEventDetails()` closes the obvious oracle (`title=like(x)` or a sort on a hidden field) with a 400. **Not enforced:** `HEAD /:id` (`exists`) and the `count` number itself say nothing about content; a private event still counts as a block. The guest list of an event hidden from *guests* is not hidden from readers of the *organizer's* calendar (a shared-calendar grantee sees a default/public event's attendees; only `private`/`confidential` are blocks). Reminders, `CalendarReminderJob` pushes and `DataExport`/`MatterExport` are the owner's own data and unaffected. A delegate with `UPDATE` can of course also edit or delete a private event (unchanged). Nothing outside this repo (ActiveSync/MAPI live in sibling repos) was touched, so they still map no description/class/guest permissions until they are updated.
- **Guest list.** `MeetingSchedulingJob.sendPerAttendee()` generalizes the per-attendee video-link path: per attendee for a `REQUEST` with `videoMeetingUid` (links, as before) and for any `REQUEST`/`CANCEL` with `guestsCanSeeGuestList: false` (each copy lists the attendee and the organizer's own attendee entry). Attendee copies enforce it a second time on the receiving side (`ScanQueueJob.processItipRequest()` and `putInviteOnCalendar()` keep only the mailbox's own entry when the file says the list is hidden and names the mailbox; with no entry of the mailbox's own the list is left as sent) and `describeInvite()` lists only the reader to a guest. **Not enforced:** a guest whose organizer runs another server (or who forwards the mail) gets whatever that organizer sent; the attendee's own mailbox can be an alias/list address that matches nothing, in which case the list is left as sent; RSVP `REPLY`s to the organizer are unchanged.
- **`POST /calendar-events/:id/request-change`** (`requestChange()`), and its inbound half (`ScanQueueJob.processItipChangeRequest()`): see the release note for the contract. Design points. (1) The mail is a plain iTIP `COUNTER` (old servers see a proposal). (2) **The organizer's own row decides**, never the flags in the message. (3) All-or-nothing: one disallowed change applies none. (4) It carries only what changed - the title always (the card shows it), the location only when requested, because a guest's copy can hold their *personalized* video link as `LOCATION` (`CalendarEventAttendeeLink`) that must never be read as a request. So a request can set but not clear a title/location/description. (5) Stale (a lower `SEQUENCE` than the organizer's row) and replays apply nothing - applying raises `sequence`, so a re-delivered message is stale by itself. (6) Applying does not touch `inviteSequenceSent`; `MeetingSchedulingJob` sees `sequence != inviteSequenceSent` and re-invites everyone. (7) The delivered message is marked `meetingResponse: "accepted"` (as `accept-proposal` marks it) via `markChangeRequestApplied()`, retried on a version conflict, only logged when it can't be recorded; `MessageInvite.changeRequest.applied` reads it back. (8) Times: the requester is set accepted and the rest needs-action, mirroring `accept-proposal`; an override row changes that occurrence, a master changes the series' first start/end (its `RRULE` is never touched by a request). (9) `guestsCanInviteOthers` defaults to `true`, so by default any guest of a meeting organized on a RapidMX server can add up to 50 guests per request automatically (Google's default); organizers who don't want it set the flag to `false`. There is no rate limit on the endpoint beyond the ordinary API limits, and the inbound side has none beyond the flags.
- **Update path.** `prepareUpdate()` normalizes the description/visibility/flags (a `null` clears the description; a `null` flag/visibility is dropped), raises `sequence` when one changes (they travel in the invitation), and *ignores* the three guest flags when the row is not organized by the mailbox (`isOrganizedByOwner()`: no organizer address, or one of the mailbox's addresses; one mailbox lookup only when the patch names a flag). `prepareCreate()` does the description/visibility/flags validation; `create()` of an attendee copy with flags is allowed (the owner's own data).
- **Tests.** Both backends: `test/routes/calendarEventDialogSuite.ts` (validation, sanitizing over HTTP, sequence bumps, the flag-ignoring rule, redaction for owner / read-list grantee / `UPDATE` delegate / share-link holder / no access, the detail-query 400s, `request-change` with every permission combination, 403/400/404/502 paths and override rows) and additions to `calendarInviteSuite.ts` (invite card fields, hidden list, filing the copy, no details in REPLY/COUNTER, change-request card) run from `{mongo,sql}/CalendarEventRoute.test.ts`; `test/jobs/meetingSchedulingLinkSuite.ts` (description/CLASS/flags mailed, hidden-list per attendee for REQUEST and CANCEL, with and without a video link) from `MeetingSchedulingJob{Mongo,SQL}.test.ts`; `test/jobs/eventDialogItipSuite.ts` (inbound REQUEST fields, hostile HTML, hidden list, and every apply/deny case of the change request) from `ScanQueueJob{Mongo,SQL}.test.ts`. Pure: `test/util/EventDescriptionUtils.test.ts` (hostile HTML), `CalendarEventUtils.test.ts`, `IcsUtils.eventDialog.test.ts`, `MeetingInviteUtils.eventDialog.test.ts`. Model constructors in `test/models/{mongo,sql}.test.ts`.
- **Reading the SQL suites:** run them one file at a time and never beside another agent's run - both share one SQLite file and `database is locked` cascades into unrelated failures (looks like flakiness in `ScanQueueJobSQL`, is not).

### 2026-09-24 (late) - two fixes found while wiring the event dialog

- **Accept after Decline** (`putInviteOnCalendar()`): `RepoUtils.delete()` is a soft delete, so a declined copy keeps the deterministic `itip:` uid and a later create failed with api-011. The route now looks the uid up with `includeDeleted` and purges a `deleted` row before filing the copy. `ScanQueueJob.processItipRequest()` files the same uid and has the same trap when a reader removed an event and the organizer later re-sends it - not changed here.
- **Invite-permission default depends on the organizer's server**: `X-RAPIDMX-GUESTS-CAN-INVITE` is only written when it differs from the default (true), so absence means true only for a file whose `PRODID` starts `-//RapidMX//` (`fromRapidMx`); for any other vendor it is false, because their server never applies a change request. `guestsCanSeeGuestList` and `guestsCanModify` defaults are unchanged.

### 2026-09-25 - Erasing the leftover data of a deleted mailbox (an address stuck after "delete")

JP (live install): created `admin@powerlevel.gg`, deleted it, then could not re-create it as a shared mailbox - `409 This address still has data from a deleted mailbox` - and the console showed nothing and offered no way out. Cause: `DELETE /mailboxes/:id` removes only the mailbox row and its own ACL; folders (ACLs parented to the mailbox uid) and every `mailboxUid` row stay, and `assertNoLeftoverMailboxData()` (folder count incl. soft-deleted, or an ACL at the uid) refuses re-creation. The documented way out, a `DataSubjectErasureRequest`, could not be used: `create()` is self-service (resolves the *caller's own* mailbox) and 404s without a row. `ErasureExecutionJob` itself already tolerated a missing mailbox row (it just skipped the row delete), so the gap was on the request side plus one leak in the job (the mailbox's own ACL).

- **Design: reuse the erasure request and job.** `POST /erasure-requests/leftover {mailboxUid}` (`BaseDataSubjectErasureRequestRoute.eraseLeftover()`) files a request already `approved` with the new `leftoverOnly` flag (column on both request models, SQL nullable so `synchronize` adds it, no migration), and the job runs it: same purge, hold checks, retention rules and audit trail. All the request/list logic is in `util/LeftoverMailboxUtils.ts` (`fileLeftoverErasure()`, `listLeftoverMailboxes()`, `findLeftoverEvidence()`), shared by the erasure route and `BaseMailboxRoute`. No new model, no new job.
- **Authorization decision.** Trusted role AND elevated token (`assertAdminScope()`, like `?scope=admin`), not just `@RequiresTrustedRole()` as `approve()` is, because it is destructive and there is nobody to consent for a mailbox that no longer exists. It is not "access to anyone's mail" (nothing is read or returned; responses are uids, counts and request metadata), and an administrator may already delete the mailbox without a grant, so erasing what that delete left is no wider. An administrator needs no grant precisely because the mailbox row, which is what grants hang off, is gone. `?erase=true` on delete is admin-only and checked before anything is deleted (an owner deleting their own mailbox gets `kept` and cannot erase - self-service erasure stays the approval workflow). Audited: `erasure_request.created`/`.approved` (`details.leftover`), `mailbox.admin-list` for the list (`details.leftover`), `mailbox.admin-delete` for the delete.
- **Never a live mailbox.** Three layers: the endpoint 409s `mailbox-exists` if the row exists; the job denies a `leftoverOnly` request found with a mailbox row at run time (`denyLeftoverRequest()`, nothing purged - a mailbox created after filing); and `removeLeftoverMailboxAcl()` re-reads the mailbox row right before removing the ACL and keeps it when the row is back. `create()` also refuses (409 `mailbox-data-erasing`) while an `approved`/`in_progress` erasure exists for the address, even once the folders are gone, closing the window in which the job's last steps could meet a new mailbox.
- **What is purged**: everything the job already purged for a live mailbox (every `mailboxUid`-scoped model incl. soft-deleted rows, plugin `@MailboxScopedData()` models, KeyVault, calendar share links per folder, blobs unless another row references them, search index documents) - and, new, the mailbox's own `AccessControlList` document when no mailbox row exists (`aclUtils.removeACL(uid, { unlessProtected: true })`; the ACL is what keeps the address taken). Folder ACLs go with each folder (`RepoUtils.delete()` removes a record's ACL). **Not swept**: an ACL document whose owning row is already gone (nothing can enumerate ACLs by parent), and rows that carry the uid but were left when an earlier erasure's per-row delete failed after the folders/ACL were gone (nothing then marks the address as leftover; a new request is refused 404). Escrow request/audit rows stay by design (as for a live mailbox).
- **Exactness.** Every purge query is `ModelUtils.literal(uid)` now (job `purgeEntityType()` and the share-link lookup by folder uid), never a parsed `eq(...)`/pattern; tests seed neighbours (`uid+"x"`, `"x"+uid`, `_` for `-`, another domain, a `%` in the local part) and assert they survive on both backends. The endpoint lowercases and trims an address (anything with an `@`) like `createMailboxes()`; any other uid is used as given.
- **`GET /mailboxes/leftover`**: distinct folder `mailboxUid`s (Mongo `$group`, SQL `GROUP BY`, keyset by uid) minus the ones with a mailbox row (`uid in (...)` per 100), `limit` 50/100, at most 5,000 uids read per call then a `next` cursor (`LEFTOVER_LIMITS` is a mutable object so tests can lower it), counts of folders and messages per listed address by the indexed `mailboxUid` (soft-deleted included), plus the newest erasure request. Only folder-backed leftovers are listable; an ACL-only one is refused on create and erasable by naming it. Mailbox route grew abstract `messageClass` and `dataSubjectErasureRequestClass` (Mongo/SQL subclasses set them); the erasure route grew `folderClass` and an injected `ACLUtils`.
- **Machine-readable 409**: `leftoverConflict(reason, message, extra)` attaches own properties to the `ApiError` (the framework serializes them): `reason: "mailbox-data-remaining" | "mailbox-data-erasing" | "mailbox-exists"`, `mailboxUid`, `erasure: { uid, status }`. Code stays `api-011` and the original message text is unchanged, so old clients still work.
- **Delete tells the caller**: `X-Mailbox-Data: kept|erasing` and `X-Erasure-Request` response headers (set only when something was left). A browser on another origin would need them in `Access-Control-Expose-Headers`; the console reads the list instead. A failure to file the erasure after the delete is logged and answered `kept` (the mailbox is already gone; the console offers the erase from the list). The bulk `DELETE /mailboxes` (truncate) is unchanged: no `erase`, no headers.
- **Known limits.** A request approved but blocked (hold, unloaded plugin) keeps the address blocked - correct, and the list shows its status. Two administrators posting at once could file two approved requests (the second finds nothing to purge) - the same non-atomic window `create()` documents. A `completed` leftover request does not stop mail delivery for the address being dropped while no mailbox exists (`ScanQueueJob.erasureDisposition()`, unchanged); a new mailbox at the address ignores requests older than itself.
- **Tests.** Both backends: `test/routes/leftoverMailboxSuite.ts` from `{mongo,sql}/LeftoverMailbox.test.ts` (every endpoint, gates, 400/404/409, holds, idempotency, paging and scan bounds, the full delete -> 409 -> list -> erase -> job -> re-create round trip with neighbour rows, ACL-only leftovers, `?erase=true`); job cases in `test/jobs/{mongo,sql}/ErasureExecutionJob*.test.ts` (exact-uid purge with ACLs, idempotence, deny when the mailbox exists, mailbox back mid-run, ACL failure retry, hold mid-cascade); `BaseMailboxRoute.test.ts` for the file-the-erasure-failed branch.
...
### 2026-09-25 - Junk/phishing reports that teach rspamd, blocked/safe senders, exact-sender filter conditions, and a purge that deletes what it should

JP: "make this better; integrate with rspamd where possible". The web client's message card had "Report junk", "Report phishing", "Block <sender>" and "Never block <sender>" that could only move a message and write `fromContains` filter rules (a substring match: `ann@x.com` also matched `joann@x.com`, and "never block" could not beat the spam filter). Same pass: the web client's permanent delete found that a purge leaves attachments and blobs behind.

- **Report: `POST /messages/:id/report`** (`BaseMessageRoute.report()`, beside `archive()`/`classify()`, no ordering issue with `/:id`). READ + UPDATE on the folder (both, like `setVerificationSeal()`; a delegate with update alone cannot see what they would be reporting). Moves via a version-checked `repoUtils.update()` (3 attempts, re-read on 409) - `findOrCreateWellKnownFolder(JUNK|INBOX)` - and sets `Message.reportedAs`/`dateReported` (new, server-managed: in `SERVER_MANAGED_MESSAGE_FIELDS`; SQL `varchar(16)`/`datetime` nullable, and the SQL column type for a `Date | null` union is `Object`, which better-sqlite3 rejects, so the class property is `Date` while the interface says `Date | null`). There was NO existing junk/phishing marker on `Message` or `ScanResult` to reuse (`inferenceClassification` is Focused/Other only). Drafts/Outbox are 400 like `archive()`. The message is moved BEFORE the filter is taught, so a slow or dead controller never blocks or reverts the move; the audit entry records `learned`/`learnSkipped`.
- **Learning is best-effort with named reasons** - `learnSkipped`: `encrypted` (checked first, nothing read), `disabled` (`mail:scan:spam:learn:enabled`, read by the route, default true), `unsupported` (no provider or no `learn`), `too_large` (JP's contract listed four reasons; a fifth was needed to tell an oversized source from an engine failure - 5 MiB, `MAX_REPORT_LEARN_BYTES`, checked with `BlobStore.size()` BEFORE `get()` so the source is never read), `failed`. `SpamScanProvider.learn?(raw, "spam"|"ham", { recipient? })`: the optional third parameter is an addition to the contract JP gave (`Deliver-To` = the mailbox's primary address; rspamd uses it as the principal recipient for a per-user classifier, and the scan already sends `Rcpt`, so without it a per-user classifier would learn under nobody). Rspamd: `POST <controller>/learnspam|learnham`, `Password` header only when configured, any 2xx OK (a message already learned answers **208**, which is what a repeated report gets). Controller default = `url` with port 11334 (`new URL(url)`; no fetch to the scan worker, which has no learn endpoint). No admin alert channel exists in this repo (searched: no AlertUtils/notification-of-admins), so phishing = the audit entry (`message.reported`, `details.kind: "phishing"`); `recordAuditLog()` also emits it as a telemetry `Event`. `AlwaysCleanSpamScanProvider` (test double) now records lessons (`learned`, `learnError`).
- **`alwaysTrustSender`**: the sender is the `From` HEADER's address (`singleFromAddress()` on the blob), NOT `Message.from.address` - a delivered message's `from.address` is the ENVELOPE sender (2026-09-15 entry). Falls back to `from.address` when the source has no usable From; a source over 5 MiB is read only for its first 64 KiB. Needs FULL on the mailbox (403 before any side effect), not just UPDATE. An encrypted message's source is still read for its From header.
- **Lists: `Mailbox.blockedSenders`/`safeSenders`** (`util/SenderListUtils.ts` holds all parsing/matching/moving; pure functions + `changeMailboxSenderList()` for the atomic version-retry write, shared by the mailbox route and `report()`). Canonical form: lowercase address or `@domain` (bare `example.com` becomes `@example.com`, so the two spellings dedupe); domain entries match the EXACT domain, never subdomains (Outlook's behaviour; decided, easy to widen later). Owner-level guard = the `freeBusyVisibility` one (`hasMailAccess(FULL)`; `validateSenderListChange()` in `validateUpdate()`, so PUT, property PUT and bulk PUT are all covered; `restrictToAdminFields()` already drops them for an admin with no grant because they are not in `ADMIN_MANAGED_FIELDS`). `PUT` with an entry on both lists is 400 (POST moves it instead) - a PUT is a whole-list write, so silently choosing which list wins would be a guess. `withAccessRole()` normalizes null to `[]` on reads. POST/DELETE endpoints: 404 without READ (as `GET /:id`, no address oracle), 403 without FULL. The DELETE path segment is URL-decoded by the framework (`%40example.com` works).
- **Enforcement (`ScanQueueJob.applySenderLists()`)** between the scan and the `ScanResult` write: blocked -> `"junk"` (From OR envelope, no authentication, marked `BLOCKED_SENDER` in `spamSymbols`, rules skipped because junk-routed); safe -> only when the message is authenticated (`this.verifiedFromAddress()`, i.e. DKIM aligned + `trusted_authserv_id`, the repo's only definition - there is no SPF check anywhere) AND the SAFE match is on the From address (an envelope-only match proves nothing: DKIM aligns with From) -> a `"junk"` verdict becomes `"deliver"` (`SAFE_SENDER`), including the fail-closed `SUSPECT` of a spam-engine outage (a missing opinion; a decision, flagged in the release notes). Never for `"quarantine"` (AV infected/error, transport-rule/policy `quarantineReason`). Both lists: authenticated safe wins, otherwise blocked. Focused Inbox: nothing changed - a rescued message is classified like any Inbox mail (safe senders are NOT forced into Focused; no trivial hook, skipped as JP allowed).
- **Filter conditions `fromEquals`/`fromDomainEquals`** (`MailFilterUtils.matchesConditions()` + `MailFilterMatchContext.fromAddress/envelopeFrom`; both header From and envelope sender, `senderAddressesOf()` falls back to the address inside `from` for a caller that only has that). Validated in `BaseMailFilterRuleRoute.prepareCreate()/prepareUpdate()` (`normalizeFilterSenderList()`, 100 entries, 254 chars). ActiveSync/MAPI mappings are in sibling repos (`activesync`, `mapi`), not here - nothing to change in this repo; they do not map these conditions yet.
- **Purge (`MessagePurgeUtils.ts`)**: `BaseScopedChildRoute` grew `beforePurge()/afterPurge()` (per delete, and per 500-record truncate batch), `afterTruncate()`, `onTruncateRefused()` and `checkLegalHold(existing, user)`. Order is ROWS FIRST, THEN BLOBS (unlike `RetentionEnforcementJob`, which deletes content first with an `exclude`): a truncate deletes many messages at once and `isBlobKeyReferenced()`'s `exclude` names one row, so two messages of the same truncate sharing a blob would each see the other as a reference and nobody would delete it. The price is the job's opposite trade-off (a crash between the row delete and the blob delete leaves an orphan blob, never a dangling reference). Best-effort per blob and per attachment row: logged, never thrown. Message-level blobs are only collected when the route knows BOTH `quarantineEntryClass` and `ingestQueueEntryClass` (a raw message is shared with other recipients' queued/quarantined copies); the Mongo/SQL routes set them. Attachments capped at 20,000 rows per batch (logged). Truncate audit = ONE `message.truncate` entry per call; a refused truncate = `legal_hold.blocked_delete` with `details.truncate`. Legal-hold 409 text: the matter uids only for a trusted-role caller (`this.isTrusted(user)`) - matters are holder-gated and even an admin is not a holder, so "entitled to see matters" could not be decided here; the status/code did not change and no existing test asserted the text. `assertNotOnLegalHold()` (mailbox delete, erasure) was left alone and still lists uids.
- **Tests (both backends unless noted)**: `test/routes/{messageReportSuite,senderListsRouteSuite,messagePurgeSuite}.ts` from `{mongo,sql}/{MessageReport,SenderListsRoute,MessagePurge}.test.ts`; `test/jobs/senderListsSuite.ts` from `ScanQueueJob{Mongo,SQL}.test.ts`; unit: `test/util/{SenderListUtils,MailFilterUtils.senders,MessagePurgeUtils}.test.ts`, `test/scan/RspamdSpamScanProvider.{learn,wire}.test.ts` (the wire one runs a real local HTTP server), `test/models/senderLists.test.ts`. The route instance the report suite flips `spamLearnEnabled` on is `objectFactory.getInstance("routes.MessageRoute")` (the test server registers the class under that fqn).
- **Server/chart work this needs (not done here)**: expose rspamd's controller (container port + Service 11334 - today only 11333), give it a password (`rspamadm pw` hash in `worker-controller.inc`, the plain value in a Secret) and a Redis statistics backend for the `bayes` classifier (learning fails without one, reported `failed`), pass `mail__scan__spam__rspamd__controller_password` (and optionally `..._controller_url`, `..._controller_timeout_ms`, `mail__scan__spam__learn__enabled`) to the server; `DevBypassSpamScanProvider` should forward `learn()` to its inner provider (until then reports are `unsupported` in dev).

### 2026-09-26 - `Plugin.configured` (what the deployment sets for a plugin's settings)

Not committed. JP: the Video Conferencing settings form showed empty TURN fields although the chart's coturn sets them through the environment. Decided order: **a setting saved in the admin console, then the environment, then the defaults; an empty or null saved value is nothing.**
- `configuredPluginSettings(config, manifest)` (`PluginUtils.ts`) walks `config.stores`, skipping the saved-settings layer (`PLUGIN_SETTINGS_STORE` = `"plugins"`, the same name as the server's `config.defaults.ts` constant - keep them equal), and returns `{ value?, secret }` per declared setting from the first layer that has a non-empty value (argv, env, memory, defaults). That is what applies with nothing saved and what Reset goes back to.
- `BasePluginRoute.withConfigured()` adds it as `configured` to `list()`, `add()` (plugin and dependencies) and `update()`; `settings` is never changed. Secrets (a key matching `/secret|password|credential|token|api_?key/i`) never carry the value. The heuristic is deliberate: a manifest flag would need every plugin to opt in, and a leaked shared secret is the failure that matters. Trusted role only, like the rest of the route.
- Tests: `configuredPluginSettings` in `test/plugins/PluginUtils.test.ts` (real nconf with the server's layers and the environment set), `withConfigured` in `test/routes/BasePluginRoute.test.ts`. **Not run: the HTTP route suites (`pluginRouteSuite.ts`) - they need a database**, so `configured` in a real response is untested; `tsc` and `eslint` are clean.

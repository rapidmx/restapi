...
describe("DataExportJobMongo Tests (real DB + DI)", () => {
    ...
    it("Marks an abandoned request failed instead of reclaiming it once max_attempts is reached.", async () => {
        ...
        expect((await auditLogRepo.find({ action: AuditAction.DATA_EXPORT_FAILED }).toArray()).length).toBe(1);
    });
    ...
    it("Fails a JSON export that exceeds mail:export:max_bytes.", async () => {
        const mailbox = await createMailbox();
        await contactRepo.save(new ContactMongo({ mailboxUid: mailbox.uid, folderUid: uuid.v4(), displayName: "x".repeat(500) }));
        const request = await createRequest({ mailboxUid: mailbox.uid, format: "json" });

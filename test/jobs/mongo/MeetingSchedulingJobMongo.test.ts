...
describe("MeetingSchedulingJobMongo Tests (real DB + DI)", () => {
    ...
    describe("Attendee copies edited/deleted by a client, and batch starvation", () => {
        ...
        const softDelete = async (uid: string): Promise<void> => {
            await calendarEventRepo.updateOne({ uid } as any, { $set: { deleted: true, dateModified: new Date() }, $inc: { version: 1 } } as any);
        };
        const clientEdit = async (uid: string, changes: Record<string, any>): Promise<void> => {
            await calendarEventRepo.updateOne({ uid } as any, { $set: { ...changes, dateModified: new Date() }, $inc: { version: 1 } } as any);
        };
        ...
        it("Sends no CANCEL when a client deletes an attendee copy of someone else's meeting, but stamps cancelNoticeSentAt.", async () => {
            ...
            expect(transport().sent.length).toBe(0);
            const updated: any = await calendarEventRepo.findOne({ uid: event.uid } as any);
            expect(updated.cancelNoticeSentAt).toBeTruthy();
        });

...
describe("ExternalShareExpirationJobSQL Tests (real DB + DI)", () => {
    ...
    it("Revokes the ACL grant before deleting the row, keeping the row for retry when revocation fails.", async () => {
        ...
        expect(await calendarShareLinkRepo.findOne({ where: { uid: expired.uid } })).not.toBeNull();

        saveSpy.mockRestore();
        await job.run();

        expect(await calendarShareLinkRepo.findOne({ where: { uid: expired.uid } })).toBeNull();
        const acl = await aclUtils.findACL(folderUid, [], { skipCache: true });
        expect(acl?.records.find((r) => r.userOrRoleId === expired.token)).toBeUndefined();
    });
});

...
describe("Route:EscrowAccessRequestMongo Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-mongo", logger, objectFactory });
    const baseUrl = "/mongo/escrow-access-requests";
    ...
    let requestRepo: MongoRepository<EscrowAccessRequestMongo>;
    let escrowAuditRepo: MongoRepository<EscrowAuditLogEntryMongo>;
    ...
    const createMatter = async function (escrowScopeId: string, custodianMailboxUids: string[], data?: any): Promise<MatterMongo> {
    ...
    beforeAll(async () => {
        ...
        if (conn instanceof MongoConnection) {
            ...
            requestRepo = conn.getMongoRepository("EscrowAccessRequestMongo");
            escrowAuditRepo = conn.getMongoRepository("EscrowAuditLogEntryMongo");
    ...
    beforeEach(async () => {
        for (const r of [requestRepo, escrowAuditRepo, auditLogRepo, keyVaultRepo, matterRepo, mailboxRepo, escrowScopeRepo]) {
            try {
                await r.clear();
            } catch (err: any) {
    ...
    describe("create()", () => {
        ...
        it("requiredHolders:1 - creation alone reaches status 'approved' and material() is immediately readable.", async () => {
            ...
            await keyVaultRepo.save(
                new KeyVaultMongo({ mailboxUid: mailbox.uid, wrappedKeys: [], masterKeyWraps: [escrowWrap(scope.uid), passwordWrap()] }),
            );
            const matter = await createMatter(scope.uid, [mailbox.uid]);
            ...
            const createdEntries = await escrowAuditRepo.find({}).toArray();
            ...
            const afterMaterialEntries = await escrowAuditRepo.find({}).toArray();
            expect(afterMaterialEntries).toHaveLength(2);

            const fulfilled = await requestRepo.findOne({ uid: createResult.body.uid } as any);
    ...
    describe("requiredHolders:2 dual-control math", () => {
        ...
        it("Stays pending after creation, rejects material() (403), rejects self re-approval (400), rejects a non-holder approve (403), then approves and allows material() once B approves.", async () => {
            ...
            const auditEntries = await escrowAuditRepo.find({}).toArray();
            expect(auditEntries.map((e) => e.action).sort()).toEqual(
                [EscrowAuditAction.REQUEST_CREATED, EscrowAuditAction.REQUEST_APPROVED].sort(),
            );
            ...
            const afterBothReads = await escrowAuditRepo.find({ action: EscrowAuditAction.MATERIAL_READ }).toArray();
    ...
    describe("deny()", () => {
        ...
        it("A holder can deny a pending request, blocking further approve/material, recorded only in the general audit log.", async () => {
            ...
            const escrowEntries = await escrowAuditRepo.find({}).toArray();
            expect(escrowEntries).toHaveLength(1); // only REQUEST_CREATED - deny is never hash-chained
            const generalEntries = await auditLogRepo.find({ targetUid: createResult.body.uid }).toArray();
    ...
    describe("Matter.delete() guard", () => {
        it("Rejects deleting a matter that an EscrowAccessRequest still references (409).", async () => {
            ...
            const result = await request(server.getApplication())
                .delete(`/mongo/matters/${matter.uid}`)
                .set("Authorization", "jwt " + holderAToken);

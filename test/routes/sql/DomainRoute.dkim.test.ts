...
// test/routes/sql/DomainRoute.test.ts (which deliberately registers no DkimKeyProvider at all, so its own
// create()/dns-setup() behavior stays exactly as it was before this feature existed) so that suite's
...
describe("Route:DomainSQL DKIM auto-generation Tests", () => {
    const logger = Logger();
    ...
    beforeAll(async () => {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "domainroute-dkim-sql-test-"));
    ...
    it("Backfills a DKIM key pair the first time dns-setup() is called on a pre-existing domain missing one.", async () => {
        ...
        expect(domain.dkimSelector).toBeFalsy();
    ...
    it("Does not regenerate a key pair for a domain that already has one.", async () => {
        ...
        expect(result.status).toBe(200);
        const refreshed = await repo.findOne({ where: { uid: created.body.uid } });

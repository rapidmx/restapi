...
describe("Route:NoteSQL Tests", () => {
    ...
    it("Rejects creating a note with an explicit empty title (400) - proves model validation actually runs on create().", async () => {
        const mailbox = await createMailbox(owner.uid);
        const folder = await createFolder(mailbox.uid);
        ...
        const result = await request(server.getApplication())
            .post(baseUrl)
            .set("Authorization", "jwt " + ownerToken)
            .send({ mailboxUid: mailbox.uid, folderUid: folder.uid, title: "", body: "Some content" });
    ...
    it("Silently corrects a client-supplied mailboxUid on update() to the note's real folder's mailbox, rather than trusting it.", async () => {
        ...
        const persisted = await noteRepo.findOne({ where: { uid: note.uid } });

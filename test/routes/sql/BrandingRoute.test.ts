...
describe("Route:BrandingSQL Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-sql", logger, objectFactory });
    const baseUrl = "/sql/branding";
    let brandingRepo: Repository<BrandingSQL>;
    ...
    beforeAll(async () => {
        ...
        if (isSqlDataSource(conn)) {
            brandingRepo = conn.getRepository(BrandingSQL);
    ...
    describe("PUT /branding (trusted role only)", () => {
        ...
        it("A trusted caller creates the singleton row on first write.", async () => {
            ...
            const rows = await brandingRepo.find();
        ...
        it("Ignores a client-supplied logoBlobKey/logoContentType - never settable directly.", async () => {
            ...
            expect(result.status).toBe(200);
            const row = await brandingRepo.findOne({ where: { uid: "branding" } });
            expect((row as any)?.logoBlobKey).toBeFalsy();
        });
        ...
        it("Records an audit log entry.", async () => {
            ...
            const entries = await auditLogRepo.find({ where: { action: AuditAction.BRANDING_UPDATE } });

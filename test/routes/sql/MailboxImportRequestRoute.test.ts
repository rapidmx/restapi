...
describe("Route:MailboxImportRequestSQL Tests", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    const server: Server = new Server({ config, basePath: "./test/server-sql", logger, objectFactory });
    const baseUrl = "/sql/mailbox-import-requests";
    ...
    describe("POST /mailbox-import-requests", () => {
        ...
        it("A trusted caller can create a request for a mailbox they hold a grant on, whoever owns it.", async () => {
            const mailbox = await createMailbox(owner.uid);
            const folder = await createFolder(mailbox.uid);
            await (objectFactory.getInstance(ConnectionManager)!.connections.get("acl") as any)
                .getRepository(AccessControlListSQL)
                .save({ uid: mailbox.uid, dateCreated: new Date(), dateModified: new Date(), version: 0, parentUid: "Mailbox", records: [{ userOrRoleId: admin.uid, actions: ["*"] }] } as any);
        ...
        it("Answers 404 for a mailbox the caller holds a grant on whose row no longer exists (a leftover ACL).", async () => {
            ...
            await (objectFactory.getInstance(ConnectionManager)!.connections.get("acl") as any)
                .getRepository(AccessControlListSQL)
                .save({ uid: ghost, dateCreated: new Date(), dateModified: new Date(), version: 0, parentUid: "Mailbox", records: [{ userOrRoleId: admin.uid, actions: ["*"] }] } as any);
        ...
        it("Records an audit log entry.", async () => {
            ...
            const entries = await auditLogRepo.find({ where: { action: AuditAction.MAILBOX_IMPORT_REQUESTED } });

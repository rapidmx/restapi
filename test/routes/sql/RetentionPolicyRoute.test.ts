...
describe("Route:RetentionPolicySQL Tests", () => {
    ...
    const baseUrl = "/sql/retention-policy";
    let policyRepo: Repository<RetentionPolicySQL>;
    ...
    beforeAll(async () => {
        ...
        if (isSqlDataSource(conn)) {
            policyRepo = conn.getRepository(RetentionPolicySQL);
    ...
    describe("PUT /retention-policy (trusted role only)", () => {
        ...
        it("Records an audit log entry.", async () => {
            ...
            const entries = await auditLogRepo.find({ where: { action: AuditAction.RETENTION_POLICY_UPDATE } });
